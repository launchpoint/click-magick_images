        ...
        if(key_exists($name, $this->loadedPlugins)) {
        	if(! is_null($version)) {
        	   if( property_exists($this->loadedPlugins[$name], 'version') ){
	        	   	...
	        	   	click_error('Plugin "'.$name.'" version ='.$this->loadedPlugins[$name]->version . ' required >= ' . $version);
        	   }
        	}
        	return true ;
        }

      click_error('Plugin "'.$name.'" not found!');
    	return false;
    }
...
//			include_once $plugin ;
	...
	public function execute($cmd){
        ...
        if($ret != 0)
        {
          click_error("PHPMagick failed", array('cmd'=>$cmd, 'out'=>$out, 'ret'=>$ret));
        }
